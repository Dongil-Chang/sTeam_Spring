...
<body>
<h3><a href="javascript:history.back(-1)"><img alt="" src="imgs/QnA.png"></a> </h3>
...
<tr><th>내용</th>
	<td class="left" colspan="5" style="text-align: justify; padding-left: 28px; padding-right: 28px;">${fn:replace(vo.q_content, crlf, '<br>')}</td>
</tr>
...
<div class="btnSet">
	...
	<a class="btn-fill" href="mqlist.mp?id=${loginInfo.id }">목록으로</a>
	</c:if>
	...
	<c:if test="${loginInfo.id eq vo.q_writer || loginInfo.admin eq 'Y'}">
		<a class="btn-fill" onclick= "if( confirm('정말 삭제하시겠습니까?') ){ href='mdelete.qa?q_id=${vo.q_id} '} ">삭제</a>
	</c:if>
	</c:if>
