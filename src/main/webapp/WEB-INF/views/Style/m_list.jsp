		...
		<%-- <ul>				
			<li>
					<select name="search" class="w-px120"  id="hair_category" onchange="$('form').submit()">
						<option value="all" ${page.search eq 'all' ? 'selected' : '' }>전체</option>
						<option value="hair" ${page.search eq 'hair' ? 'selected' : '' }>헤어제품</option>
						<option value="care" ${page.search eq 'care' ? 'selected' : '' }>헤어케어</option>
						<option value="wstyle" ${page.search eq 'wstyle' ? 'selected' : '' }>여성스타일링</option>						
						<option value="mstyle" ${page.search eq 'mstyle' ? 'selected' : '' }>남성스타일링</option>						
					</select>
				</li>	
		</ul> --%>
...
<tbody>
	...
	<tr style="border: 1px solid #b0b0b0; "><td>${vo.no}</td>
		<td>${vo.type}</td>
		<td class='left' style="text-align:justify; "><a onclick='go_detail(${vo.id})'>${vo.title}</a></td>
