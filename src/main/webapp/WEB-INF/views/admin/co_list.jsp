...
.box select { /* height: 43px; */ width: 100px; text-align: center;}

.box select option { text-align: center;}

.box ul li:nth-child(3) { margin: 0px 0px 5px 5px; }

.box ul li:nth-child(3) a { margin: 0px 0px 0px 5px; height: 37px; }
...
<form method="post" action='co_list.cu'>
...
<div class="box">
	<ul>
		<li>
			...
			<option value='title' ${CoPage.search eq 'title' ? 'selected' : ''}>제목</option>
			<option value='content' ${CoPage.search eq 'content' ? 'selected' : ''}>내용</option>
			
			<c:if test="${loginInfo.admin ne 'Y' }">
			<option value='writer' ${CoPage.search eq 'writer' ? 'selected' : ''}>작성자</option>
			</c:if>
			</select>
		</li>
		<li><input type='text' name='keyword' value='${CoPage.keyword}' class='w-px300'/></li>
	...
	<ul>
		...
		<li>
			...
			<option value='10' ${CoPage.pageList eq 10 ? 'selected' : ''}>10개씩</option>
			<option value='15' ${CoPage.pageList eq 15 ? 'selected' : ''}>15개씩</option>
			<option value='20' ${CoPage.pageList eq 20 ? 'selected' : ''}>20개씩</option>
			<option value='25' ${CoPage.pageList eq 25 ? 'selected' : ''}>25개씩</option>
			<option value='30' ${CoPage.pageList eq 30 ? 'selected' : ''}>30개씩</option>
			</select>
		</li>
	</ul>
</div>
...
<c:if test="${CoPage.viewType eq 'grid'}">
<ul class='grid'>
	<c:forEach items="${CoPage.list}" var="vo">
	<li><div><a onclick='go_detail(${vo.id}, "${u_id }", "${CoPage.curPage }", "${CoPage.search }", "${CoPage.keyword }", "${name }")'>${vo.title }</a></div>
		<div>${vo.name}</div>
...
<c:if test="${CoPage.viewType eq 'list'}">
<table class='tb_list'>
<thead>
...
<tbody>
	<c:if test="${empty CoPage.list}">
	<tr><td colspan='6' class="empty">등록된 게시글 정보가 없습니다</td></tr>
	</c:if>
	
	<c:forEach items="${CoPage.list}" var="vo">
	<tr class="item"><td>${vo.no}</td>
		<td class='left'><a onclick='go_detail(${vo.id}, "${u_id }", "${CoPage.curPage }", "${CoPage.search }", "${CoPage.keyword }", "${name }")'>${vo.title}</a></td>
		<td>${vo.name}</td>
		<td>${vo.writedate}</td>
